...
- [ ] Create `app/tools/builtin/sessions.py` with all 4 tools
- [ ] Implement policy scoping for spawned sub-agents
- [ ] Register tools with correct safety classifications
- [ ] Session access control: check permissions before cross-session access

### Backend — Sub-Agent
- [ ] Create `app/agent/sub_agent.py`
- [ ] Implement spawn with policy inheritance
- [ ] Auto-cleanup for completed sub-agent sessions
- [ ] Concurrency limit enforcement (max 5 active sub-agents)

### Backend — Workflow Runtime
- [ ] Create `app/workflows/runtime.py` — execution engine
- [ ] Implement pipeline mode (sequential execution)
- [ ] Implement parallel mode (asyncio.gather)
- [ ] Workflow state machine with transitions
- [ ] Step-level error handling (retry, skip, abort)
- [ ] Progress tracking and event emission

### Backend — Workflow API
- [ ] Create `app/workflows/models.py` — Workflow, WorkflowStep, WorkflowRun
- [ ] Create `app/workflows/api.py` — full CRUD + run endpoints
- [ ] DB migration for workflow tables

### Frontend — Workflow UI
- [ ] Workflow list view
- [ ] Workflow builder (step sequencer, mode selector, agent picker)
- [ ] Run monitor (live progress, step results)
- [ ] Run history view

### Tests
- [ ] `tests/unit/test_session_tools.py` — list, history, send, spawn
- [ ] `tests/unit/test_sub_agent.py` — spawn, policy inheritance, cleanup
- [ ] `tests/unit/test_workflow_runtime.py` — pipeline, parallel, error handling
- [ ] `tests/integration/test_workflow_e2e.py` — create workflow → run → results
- [ ] `tests/integration/test_multi_agent.py` — parent spawns sub-agent, delegates, retrieves results
...
- [ ] Session tools operational: list, history, send, spawn
- [ ] Sub-agent spawning works with policy inheritance
- [ ] Workflow pipeline mode executes steps sequentially
- [ ] Workflow parallel mode executes steps concurrently
- [ ] Workflow API: full CRUD + run/cancel/status
- [ ] Workflow UI: build, run, monitor workflows
- [ ] All tests pass
- [ ] **Phase 3 gate**: Multi-agent collaboration and workflow orchestration operational
