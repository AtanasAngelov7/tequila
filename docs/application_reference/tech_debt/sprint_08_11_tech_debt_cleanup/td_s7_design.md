...
Clean up remaining design debt: implement the health-check loop, fix the contradiction step placeholder, improve NER quality, remove dead code, fix minor type issues, and tighten up test hygiene. After this sub-sprint, the codebase has no outstanding tech debt from Sprints 08â€“11.
...
| TD-122 | Truncated UUID step IDs â€” collision risk | **Low** | `app/workflows/models.py` |
...
**File**: `app/knowledge/sources/registry.py` (~lines 68â€“77)

- [x] Add a `start()` method that creates the background task:
  ```python
  ...
  ```
- [x] Call `start()` during app startup (in `app/api/app.py` lifespan)
- [x] Add a `stop()` method that cancels the task for clean shutdown

### T2: Fix `total` field to return real DB count (TD-72)

**File**: `app/workflows/api.py` (~lines 123, 183)

- [x] Add a `SELECT COUNT(*)` query for the unfiltered total:
  ```python
  total_row = await db.execute("SELECT COUNT(*) as cnt FROM workflows")
  total = total_row["cnt"]
  ```
- [x] Return this as the `total` field instead of `len(results)`

### T3: Validate `agent_id` existence at spawn/creation time (TD-73)

**Files**: `app/agent/sub_agent.py`, `app/workflows/api.py`

- [x] Before spawning a sub-agent or creating a workflow, verify the agent exists:
  ```python
  agent = await agent_store.get(agent_id)
  if not agent:
      raise ValueError(f"Agent '{agent_id}' not found")
  ```
- [x] This catches typos early instead of failing at turn-loop time

### T4: Document `update_note` title-filename behavior (TD-76)

**File**: `app/knowledge/vault.py`

- [x] Add a docstring explaining the intentional decision:
  ```python
  async def update_note(self, ...):
      ...
      """
  ```
- [x] If rename behavior is desired in the future, add it with proper conflict handling

### T5: Fix `delete_note` order â€” file first, then DB (TD-77)

**File**: `app/knowledge/vault.py` (~lines 370â€“373)
...
- [x] Reverse the order:
  ...
  ```
- [x] This avoids orphan files that `sync_from_disk` would resurrect

### T6: Improve entity mention extraction (TD-84)

**File**: `app/memory/entities.py` (~lines 136â€“180)

- [x] Expand the stopword list with common false positives:
  ```python
  _MENTION_STOPWORDS = {
  ...
  ```
- [x] Filter out single-word mentions that are in the stopword set
- [x] Optionally add a minimum length filter (e.g., 2+ characters)

### T7: Add minimal contradiction detection placeholder (TD-87)

**File**: `app/memory/extraction.py` (~lines 372â€“376)

- [x] The step currently returns candidates unchanged. Add a TODO and log:
  ```python
  async def _step4_contradiction(self, candidates, existing_memories):
      """Placeholder for contradiction detection â€” logs when enabled.

      TODO: Implement actual contradiction detection using embedding similarity
      and LLM comparison. Currently a no-op.
      """
      logger.debug("Contradiction detection step â€” not yet implemented (%d candidates)", len(candidates))
      return candidates
  ```
- [x] Update the pipeline documentation to be honest about the 5-step (not 6-step) pipeline

### T8: Cap `_step1_classify` fallback (TD-113)

**File**: `app/memory/extraction.py` (~lines 307â€“308)

- [x] Instead of returning ALL messages on LLM failure, cap to the most recent N:
  ```python
  MAX_FALLBACK_MESSAGES = 10

  except Exception:
      logger.warning("Classification step failed â€” falling back to last %d messages", MAX_FALLBACK_MESSAGES)
      return messages[-MAX_FALLBACK_MESSAGES:]
  ```

### T9: Fix `prefetch_background` to use `touch()` (TD-90)

**File**: `app/memory/recall.py` (~lines 347â€“352)

- [x] After TD-S4 T4 creates the `touch()` method, update:
  ```python
  ...
  ```
- [x] Note: This task depends on TD-S4 being complete

### T10: Generate unique session ID per `memory_extract_now` call (TD-104)

**File**: `app/tools/builtin/memory.py` (~lines 738â€“741)

- [x] Replace hardcoded fake session ID:
  ```python
  import uuid
...
- [x] Remove `from app.exceptions import NotFoundError` if unused
...
**File**: `app/knowledge/vault.py` (~lines 168â€“177)

- [x] Add a maximum iteration count:
  ```python
...
**File**: `app/knowledge/vault.py` (~lines 423â€“425)

- [x] Store the result and reuse:
  ```python
...
- [x] Change from 8 hex chars to 16:
  ```python
  step_id: str = Field(default_factory=lambda: uuid.uuid4().hex[:16])
  ```
- [x] 16 hex chars = 64 bits â†’ birthday collision at ~4 billion vs ~65K

### T16: Use `parent_id` as bucket key for sub-agent tracking (TD-124)

**File**: `app/agent/sub_agent.py` (~lines 97â€“98)

- [x] Replace `"_global"` fallback with `parent_id` or `"_orphan"`:
  ```python
...
**File**: `app/workflows/api.py` (~lines 109â€“113)

- [x] Remove the manual `if mode not in (...)` check in the handler â€” Pydantic already validates this on the request model
- [x] Keep only the Pydantic validation

### T18: Add public API for sub-agent active count (TD-126)

**File**: `app/agent/sub_agent.py`

- [x] Add a public method:
  ```python
  ...
  ```
- [x] Update test files to use this method instead of poking `_active` directly:
...
- [x] Rename `_events_router` â†’ `events_router`
- [x] Update the import in `app/api/app.py` line ~315

### T20: Extract content truncation to constant (TD-131)

**File**: `app/memory/extraction.py` (~lines 77, 93)

- [x] Define a constant at module top:
  ```python
  EXTRACTION_CONTENT_MAX_CHARS = 500
  ```
- [x] Replace hardcoded `500` with the constant

### T21: Improve token estimation for non-ASCII (TD-132)

**File**: `app/memory/recall.py` (~lines 49â€“51)
...
- [x] Replace:
  ...
  ```
- [x] This gives a more accurate estimate for CJK and other multi-byte text

### T22: Remove unused `asyncio` import (TD-133)

**File**: `app/tools/builtin/knowledge.py` (~line 4)

- [x] Remove `import asyncio` if unused

### T23: Fix `agent_id=None` type mismatch (TD-134)

**File**: `app/tools/builtin/knowledge.py` (~lines 62â€“65)

- [x] Change the function signature to accept `str | None`:
  ```python
  async def kb_search(query: str, ..., agent_id: str | None = None) -> ...:
  ```
- [x] Or pass `""` instead of `None` if the downstream function expects `str`
...
- [x] All knowledge source registry tests pass
- [x] All workflow tests pass
- [x] All sub-agent tests pass
- [x] All vault tests pass
- [x] All extraction tests pass
- [x] All recall tests pass

### New tests to add
- [x] Test health-check loop starts and runs (mock adapter)
- [x] Test `total` field returns correct DB count for workflows/runs
- [x] Test that invalid `agent_id` raises at spawn time
- [x] Test `_unique_slug` raises after max attempts
- [x] Test `get_active_count()` public API
- [x] Test that classify fallback returns at most MAX_FALLBACK_MESSAGES

---

## Definition of Done

- [x] All 23 items resolved
- [x] Health-check background task starts on app boot
- [x] No dead code or unused imports in Sprint 08â€“11 files
- [x] NER stopword list expanded
- [x] All `type: ignore` comments in scope are resolved
- [x] Tests use public APIs (no private dict access)
- [x] All existing tests pass (683+, 1 pre-existing failure)
- [x] New tests added where specified
- [x] **All 95 tech debt items from Sprint 08â€“11 audit are resolved**
